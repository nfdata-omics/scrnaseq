process QUALITY_FILTERING_ATAC  {
    ...
    qualitymetricsfilters_atac.py  -fr ${fragment_files.join(' ')} -fri ${fragment_index_files.join(' ')}  -id ${meta.collect{ m -> m.id }.join(' ')} -t $tss_threshold -mif $min_fragments_counts -maf $max_fragments_counts -b $blacklist_path -g $genome_annotation
