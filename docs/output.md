...
- `${meta.id}/qcatch/${meta.id}_qcatch_report.html` (unless `--skip_qcatch`)
  - HTML QC report from [qcatch](https://github.com/AltosLabs/qcatch) for each sample.
- `${meta.id}/qcatch/${meta.id}_filtered_quants.h5ad` (unless `--skip_qcatch`)
  - Optional filtered counts in h5ad format (`--remove_doublets` adds doublet filtering).
- `${meta.id}/qcatch/${meta.id}_metrics_summary.csv` (unless `--skip_qcatch`)
  - Per-sample metrics table exported by qcatch.
- `simpleaf_index`
