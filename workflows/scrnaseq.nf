...
workflow SCRNASEQ {
    ...
    ch_vdj               = params.vdj ? file(params.vdj, checkIfExists: true)       : []
    ...
    if (params.vdj) {
        ...
        ch_vdj_input = Channel
            ...
            .map { row ->
                ...
                def vdj_file = file(row.csv)
                tuple(meta, vdj_file)
            }
            .collect()
            .map { rows ->
                def pairs = rows.collate(2)
        ...
        ch_vdj_input.view { x -> "VDJ INPUT TO CONCATENATE_VDJ = ${x}" }
    ...
    } else if (params.aligner == "cellrangermulti" && params.has_vdj) {
    ...
    ch_vdj = ch_vdj.ifEmpty { [[id: 'dummy'], []] }
    ...
    if (params.aligner == "cellrangermulti" || params.aligner == "cellrangerarc" || params.aligner == "cellranger" ) {
        // If counts is defined, and skip_cellbender is true, use H5AD_CONVERSION.out.h5ad_filtered
        def ch_h5ad_selected = (params.counts && params.skip_cellbender) ?
            H5AD_CONVERSION.out.h5ad_filtered :
            (
                params.counts ?
                    H5AD_CONVERSION.out.h5ad_cellbender :  // counts defined and skip_cellbender false
                    (
                        params.h5ad_matrix ?
                            Channel
    ...
    def ch_rds_selected = (params.counts && !params.skip_cellbender) ? H5AD_CONVERSION.out.rds_cellbender : H5AD_CONVERSION.out.rds_concat
    ...
    ch_h5ad_raw = params.h5ad_matrix ?
        ...
        H5AD_CONVERSION.out.h5ad_raw.ifEmpty { [[:], file("${projectDir}/assets/EMPTY")] }
